...
export function useModalOpen(modal: ApplicationModal): boolean {
  const openModal = useSelector(
    (state: AppState) => state.gapplication.openModal
  );
...
export function useActivePopups(): AppState["gapplication"]["popupList"] {
  const list = useSelector((state: AppState) => state.gapplication.popupList);
